...
function Certificate() {
  ...
  const getCertificates = async () => {
    ...
    setCertificates(response.data.output.certificates);
    setData(response.data);
    console.log(response.data.output.certificates);
  };
  ...
  const handleFilter = () => {
    ...
    filteredData = [
      ...filteredData.filter((item) => {
        let filterTimeType =
          filters.timeType === "startDt" ? item.startDt : item.expiredDt;
  ...
  const renderDate = (currentData) => {
    currentData = currentData?.map((item) => ({
      ...item,
      startDt: dayjs(item?.startDt).format("MM.DD.YYYY"),
      expiredDt: dayjs(item?.expiredDt).format("MM.DD.YYYY"),
    }));
        ...
        <div className="certificate__items pb__30">
          <div className="certificate__item">
            <div className="certificate__item--count">
              Total sertifikatlar: {data.output?.certificatesCount}
